...
class PajakSource(Base, BaseModel):
...
# Synchronizer #
################
class TPPayment(Base, BaseModel):
    __tablename__ = 'tp_pajak_payment'
    pajak_payment_id = Column(Integer, nullable=False)
    jns_sinkron = Column(Integer, nullable=False)


############
# ISO 8583 #
############
class IsoInquiry(Base, BaseModel):
...
iso_payment_seq = Sequence('iso_payment_id_seq')

class IsoPayment(Base, BaseModel):
    __tablename__ = 'iso_payment'
    id = Column(Integer, iso_payment_seq, primary_key=True) 
    tgl = Column(DateTime, default=datetime.now, nullable=False)
    iso_request = Column(String(1024), nullable=False) # Untuk reversal
    payment_id = Column(Integer, ForeignKey('pajak_payment.id'), nullable=False)
    transmission = Column(DateTime, nullable=False)
    stan = Column(Integer, nullable=False)
    settlement = Column(Date, nullable=False)
