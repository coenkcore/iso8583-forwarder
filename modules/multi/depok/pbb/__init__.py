...
BANK_FIELDS = ('kd_kanwil_bank', 'kd_kppbb_bank', 'kd_bank_tunggal',
...
class InquiryResponse(BaseResponse):
    ...
    def ack_already_paid(self):
        pay = query.invoice2payment(self.calc.invoice)
        if pay and self.is_transaction_owner(pay):
            ntp = pay.id
        else:
            ntp = ''
        self.parent.set_ntp(ntp)
        self.parent.ack_already_paid()
...
class PaymentResponse(InquiryResponse):
    ...
    def create_payment(self, inq):
        ...
        iso_pay.ke = urutan
        iso_pay.kd_kanwil_bank = pay.kd_kanwil_bank
        iso_pay.kd_kppbb_bank = pay.kd_kppbb_bank
        iso_pay.kd_bank_tunggal = pay.kd_bank_tunggal 
        iso_pay.kd_bank_persepsi = pay.kd_bank_persepsi 
