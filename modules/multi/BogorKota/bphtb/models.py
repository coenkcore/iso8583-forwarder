...
class Models(object):
    def __init__(self, Base, transaction_schema, area_schema, iso_schema):
        ...
        class IsoPayment(Base, CommonModel):
            ...
            __table_args__ = dict(schema=iso_schema)
            id = Column(
                    Integer, ForeignKey('bphtb.bphtb_bank.id'),
                    primary_key=True)
        ...
        class IsoReversal(Base, CommonModel):
            ...
            __table_args__ = dict(schema=iso_schema)
            id = Column(
                    Integer, ForeignKey('bphtb_payment.id'), primary_key=True)
            tgl = Column(DateTime(timezone=True),
                         nullable=False,
                         default=create_now)
            iso_request = Column(String(1024), nullable=False)
