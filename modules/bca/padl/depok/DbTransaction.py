...
class DbTransaction(Transaction):
    ...
    def get_customer(self):
        q_cust_usaha = OtherDBSession.query(CustomerUsaha).\
                        filter_by(id=self.calc.invoice.customer_usaha_id)
        cust_usaha = q_cust_usaha.first()
        q_cust = OtherDBSession.query(Customer).\
                filter_by(id=cust_usaha.customer_id)
        cust = q_cust.first()
        if not cust_usaha.kelurahan_id:
            return cust, alamat2, cust_usaha
        q_kel = OtherDBSession.query(Kelurahan).\
                    filter_by(id=cust_usaha.kelurahan_id)
        kel = q_kel.first()
        q_kec = OtherDBSession.query(Kecamatan).\
                    filter_by(id=kel.kecamatan_id)
        kec = q_kec.first()
        alamat2 = ','.join([kel.kelurahannm, kec.kecamatannm])
        return cust, alamat2, cust_usaha

    def get_rek(self):
        q_pajak = OtherDBSession.query(Pajak).\
                    filter_by(id=self.calc.invoice.pajak_id)
        pajak = q_pajak.first()
        q_rek = OtherDBSession.query(Rekening).filter_by(id=pajak.rekening_id)
        return q_rek.first()
